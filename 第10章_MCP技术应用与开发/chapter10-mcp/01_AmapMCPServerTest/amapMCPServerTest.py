    ...
    async with stdio_client(server_params) as (read, write):
        ...
        async with ClientSession(read, write) as session:
            try:
                ...
                # maps_regeocode = await session.call_tool("maps_regeocode", arguments={"location":"114.05571,22.52245"})
                ...
                # maps_geo = await session.call_tool("maps_geo", arguments={"address":"广东省深圳市福田区"})
                ...
                maps_weather = await session.call_tool("maps_weather", arguments={"city":"成都"})
            ...
            # print(f"maps_text_search:{maps_text_search.content[0].text}/n/n")
                ...
                # 11、查询关键词搜或者周边搜获取到的 POI（Point of Interest），兴趣点 ID的详细信息，一般和 maps_text_search 结合使用
                ...
                # 12、周边搜，根据用户传入关键词以及坐标 location，搜索出 radius半径范围的 POI
