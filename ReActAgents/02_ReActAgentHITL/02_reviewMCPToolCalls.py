...
async def run_agent():
    ...
    # 1、非流式处理查询
    agent_response = await agent.ainvoke({"messages": [HumanMessage(content="查询上海的天气")]}, config)
    # 将返回的messages进行格式化输出
    ...
    agent_response = await agent.ainvoke(
        ...
        # Command(resume=[{"type": "edit", "args": {"args": {'city': '深圳'}}}]),
    ...
    #         input={"messages": [HumanMessage(content="调用工具查询上海的天气")]},
    #         config=config,
    ...
    # async for message_chunk, metadata in agent.astream(
    #     # Command(resume=[{"type": "accept"}]),
    #     Command(resume=[{"type": "edit", "args": {"args": {'city': '长沙'}}}]),
    #     # Command(resume=[{"type": "response", "args": "我不想查询了"}]),
    #     config,
    #     stream_mode="messages"
    # ):
